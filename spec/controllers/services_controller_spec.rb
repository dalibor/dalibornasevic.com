...
describe ServicesController, "lastfm" do
  ...
  before(:each) do
    Scrobbler::User.stub!(:new).with(LASTFM_USERNAME).and_return(@lastfm_user = mock('Scrobbler User'))
    @lastfm_user.stub!(:recent_tracks).and_return(@recent_tracks = mock('recent tracks'))
    @recent_tracks.stub!(:first).with(LASTFM_NUMBER_OF_TRACKS).and_return(@tracks = [mock('track')])
  end

  it "should be successful" do
    Scrobbler::User.should_receive(:new).with(LASTFM_USERNAME).and_return(@lastfm_user)
...
describe ServicesController, "twitter" do
  ...
  before(:each) do
    ...
    @twitter_search.stub!(:from).with(TWITTER_USERNAME).and_return(@twitter_search_from = mock('Twitter Search From'))
    ...
    @results.stub!(:first).with(TWITTER_NUMBER_OF_TWITS).and_return(@twits = mock('twits'))
  end
  
    it "should be successful" do
      Twitter::Search.should_receive(:new).and_return(@twitter_search)
      @twitter_search.should_receive(:from).with(TWITTER_USERNAME).and_return(@twitter_search_from)
      @twitter_search_from.should_receive(:fetch).and_return(@mash_object)
      @mash_object.should_receive(:results).and_return(@results)
      @results.should_receive(:first).with(TWITTER_NUMBER_OF_TWITS).and_return(@twits)
      get 'twitter'
      response.should be_success
